...
class PickAndScale(caffe.Layer):
    ...
    def backward(self, top, propagate_down, bottom):
        for i, (j, s) in enumerate(self.dims):
            bottom[0].diff[:, i, :, :] = top[0].diff[:, j, :, :] *s    
            
class NormAndScale(caffe.Layer):
    def setup(self, bottom, top):
        self.nch_out = len(self.param_str.split('_'))
        self.dims = []
        for f in self.param_str.split('_'):
    ...
    def forward(self, bottom, top):
        norm = np.linalg.norm(bottom[1].data)
        for i, (j, s) in enumerate(self.dims):
            top[0].data[:, i, :, :] = bottom[0].data[:, j, :, :] * s/ norm

    def backward(self, top, propagate_down, bottom):
        norm = np.linalg.norm(bottom[1].data)
        for i, (j, s) in enumerate(self.dims):
            bottom[0].diff[:, i, :, :] = top[0].diff[:, j, :, :] *s /norm 
            
class PickAndScale_noback(caffe.Layer):
    def setup(self, bottom, top):
        self.nch_out = len(self.param_str.split('_'))
        self.dims = []
        for f in self.param_str.split('_'):
...
class GraphFeature(caffe.Layer):
    def setup(self, bottom, top):
    ...
    def reshape(self, bottom, top):
        top[0].reshape(bottom[0].data.shape[0], bottom[0].data.shape[1], bottom[0].data.shape[2]*(self.numNN + 1) , bottom[0].data.shape[3])
        
    def forward(self, bottom, top):
        for ind in range(bottom[0].data.shape[0]):
            for i in range(self.numpts):
                top[0].data[ind, :, 0, i] = bottom[0].data[ind, :, 0, i]
                for j in range(self.numNN ):
                    top[0].data[ind, :, j+1, i] = bottom[0].data[ind, :, 0, self.nn_idx[ind , i, j]]  - bottom[0].data[ind, :, 0, i]
        
    def backward(self, top, propagate_down, bottom):
        for ind in range(bottom[0].data.shape[0]):
            for i in range(self.numpts):
                top[0].diff[ind, :, 0, i] = bottom[0].diff[ind, :, 0, i]
                for j in range(self.numNN ):
                    top[0].diff[ind, :, j+1, i] = bottom[0].diff[ind, :, :, self.nn_idx[ind , i, j]]  - bottom[0].diff[ind, :, :, i]