...
def extract_feat_shapes(part_label, network_path, weights_path, feed, out_names, batch_size=64, sample_size=3000):
    if sample_size == -1:
        assert batch_size == 1
    ...
    for b in range(int(np.ceil(nsamples / batch_size))):
        ...
        '''np.save("npy/data_" + str(b), net.blobs["data"].data)  
        np.save("npy/feature2_"+ str(b), net.blobs["conv2"].data)
        np.save("npy/feature3_"+ str(b), net.blobs["conv3"].data)
        np.save("npy/feature4_"+ str(b), net.blobs["conv4"].data)
        np.save("npy/feature5_"+ str(b), net.blobs["conv5"].data)
        np.save("npy/feature6_"+ str(b), net.blobs["conv6"].data)
        np.save("npy/feature6_"+ str(b), net.blobs["conv6"].data)
        np.save("npy/bn1_"+ str(b), net.blobs["bn1"].data)
        np.save("npy/bn2_"+ str(b), net.blobs["bn2"].data)
        np.save("npy/bn3_"+ str(b), net.blobs["bn3"].data)
        np.save("npy/bn4_"+ str(b), net.blobs["bn4"].data)
        np.save("npy/bn5_"+ str(b), net.blobs["bn5"].data)
        np.save("npy/bn6_"+ str(b), net.blobs["bn6"].data)'''
    if sample_size != -1:
    ...
    return outs['prob'], part_label, feed['data']
...
def partseg_test(dataset, network, weights, input_dims='x_y_z', sample_size=3000, batch_size=64,
    ...
    if dataset == 'shapenet':
        ...
        import splatnet.dataset.dataset_lidar as lidar
        dataset_params_new = {} if not dataset_params else dataset_params
        dataset_params = dict(subset='test')  # default values
        dataset_params.update(dataset_params_new)

        data, part_label, names = lidar.points_test_single_area(dims=input_dims, test_area=category, **dataset_params)
        xyz_norm_list, _, _ = lidar.points_test_single_area(dims='x_y_z_r_g_b', test_area=category, **dataset_params)
        cmap = splatnet.configs.g_label2color
    else:
        raise ValueError('Unsupported dataset: {}'.format(dataset))

    tic = time.time()
    probs, selected_label, selected_data = extract_feat_shapes(part_label, network, weights,
                                feed=dict(data=data),
                                out_names=('prob',),
    ...
    acc, avgacc, avgiou = seg_scores(preds, selected_label, nclasses=13)

    
    # save names (hashlist)
    with open("names.pickle", "wb") as fp:   #Pickling
        pickle.dump(names, fp)
    np.save("avg_iou", np.asarray(avgiou))    
    if not skip_ply:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        for xyz_norm, pred, name in zip(selected_data, preds, names):
...
end_header'''.format(len(pred))
        ...
        for xyz_norm, pred, name in zip(selected_data, selected_label, names):
            out = np.array([np.concatenate((x, cmap[int(c)]), axis=0) for (x, c) in zip(xyz_norm, pred)])
            header = '''ply
format ascii 1.0
element vertex {}
...
end_header'''.format(len(pred))
            fmt = '%.6f %.6f %.6f %.6f %.6f %.6f %d %d %d'
            save_path = os.path.join(save_dir, '{}_gt.ply'.format(name))
            np.savetxt(save_path, out, fmt=fmt, header=header, comments='')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', default='x_y_z_r_g_b', help='features to use as input')
