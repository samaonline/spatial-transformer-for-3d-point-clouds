...
from splatnet.configs import SN_CATEGORIES, SN_CATEGORY_NAMES, SN_NUM_PART_CATEGORIES, LIDAR3D_DATA_DIR #
import pdb
...
def points_test_single_area(subset, test_area=1,
                           dims='x_y_z',    # combinations of 'x', 'y', 'z', 'nx', 'ny', 'nz' and 'one'
                           read_cache=False, write_cache=False, cache_dir='',
                           shuffle=False,
                           root=LIDAR3D_DATA_DIR):
    if not cache_dir:
    ...
    if read_cache or write_cache:
        ...
        cache_path = os.path.join(cache_dir, '{}_{}.cache'.format(subset, test_area))
    ...
    if not feat_list:
        ...
        if subset == "train":
            hash_list = sorted([s[:-4] for s in filter(lambda s: s.endswith('.npy') and "Area_"+str(test_area) not in s, os.listdir(root))]) #change back!!
        else:
            hash_list = sorted([s[:-4] for s in filter(lambda s: s.endswith('.npy') and "Area_"+str(test_area) in s, os.listdir(root))])
        for s in hash_list:
            data = np.load( os.path.join(root, s + '.npy') )
...
class InputShapenet(caffe.Layer):
    ...
    def setup(self, bottom, top):
        params = dict(subset='train', test_area="1", batch_size=32, sample_size=3000,
        ...
        params['root'] = LIDAR3D_DATA_DIR
        data, label, _ = points_test_single_area(params['subset'], int(params['test_area']),
    ...
    def reshape(self, bottom, top):
        for top_index, name in enumerate(self.top_names):
            ...
            if top_index ==1:
                shape = (self.batch_size, 1, 1, self.sample_size)
            else:
                shape = (self.batch_size, 9, 1, self.sample_size)
    ...
    def forward(self, bottom, top):
        #temp_data, temp_label = self.data[self.index:self.index+self.batch_size], self.label[self.index:self.index+self.batch_size]
        temp_data, temp_label = np.concatenate(self.data[self.index:self.index+self.batch_size], axis=0), np.concatenate(self.label[self.index:self.index+self.batch_size], axis=0)
        temp_data = temp_data.reshape(self.batch_size, self.sample_size, -1, 1).transpose(0, 2, 3, 1)
        temp_label = temp_label.reshape(self.batch_size, self.sample_size, -1, 1).transpose(0, 2, 3, 1)
