...
for CAT in $CATS; do
mkdir -p area$CAT
TEMP1="_net_deform_nobn.prototxt"
TEMP2=".caffemodel"
python $SPLT_CODE/partseg3d/train.py $EXP_DIR/area$CAT \
    --categories $CAT \
    ...
    --prefix net_deform_lr1e-4 \
    --base_lr 1e-4 --lr_decay 0.2 --stepsize 2000 --num_iter 20000 --test_interval 50 --snapshot_interval 50 --iter_size 10 \
    2>&1 | tee $EXP_DIR/area$CAT/train_net_deform_nobn_lr1e-4.log; done ;
fi

# test & plot
if [ $SKIP_TEST -le 0 ]; then
python $SPLT_CODE/partseg3d/test.py shapenetB \
