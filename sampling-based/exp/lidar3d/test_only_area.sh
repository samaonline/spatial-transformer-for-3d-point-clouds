...
if [ -z $SKIP_TRAIN ]; then SKIP_TRAIN=1; fi
if [ -z $SKIP_TEST ]; then SKIP_TEST=0; fi
#if [ -z "$CATS" ]; then CATS="motorbike"; fi
...
python $SCRIPT_DIR/../../tools/pick_best_model.py $EXP_DIR best_loss
fi

# test & plot
CAT="6"
TEMP1="_netm6_sumTHA_d.prototxt"
TEMP2="_netm6_sumTHA_lr1e-5_iter_2600.caffemodel"

if [ $SKIP_TEST -le 0 ]; then
srun --mpi=pmi2 --gres=gpu:1 -n1 --ntasks-per-node=1 --job-name=bilateral --kill-on-bad-exit=1 --partition Test python $SPLT_CODE/partseg3d/test2.py lidar \
    --dataset_params root $SPLT_DATA/stanford_indoor3d_norm6 \
    --categories $CAT \
    --network area$CAT$TEMP1 \
    --weights /mnt/lustre/wangjiayun/repos/splatnet/exp/lidar3d/area$CAT/$CAT$TEMP2 \
    --sample_size 4096 --batch_size 1 \
    --exp_dir $EXP_DIR ;
fi
