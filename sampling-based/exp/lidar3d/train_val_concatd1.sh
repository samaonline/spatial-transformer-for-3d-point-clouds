...
for CAT in $CATS; do
mkdir -p area$CAT
TEMP1="_netm_sum_d1.prototxt"
TEMP2=".caffemodel"
srun --mpi=pmi2 --gres=gpu:4 -n1 --ntasks-per-node=1 --job-name=sumd1 --kill-on-bad-exit=1 --partition hkres python $SPLT_CODE/partseg3d/train.py $EXP_DIR/area$CAT \
    --categories $CAT \
